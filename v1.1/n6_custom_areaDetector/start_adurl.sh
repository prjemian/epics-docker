...
IOC_MANAGER=/opt/iocURLDriver/adurl.sh
...
IOC_TOP=${HOST_IOC_ROOT}/iocURLDriver
...
docker cp ${CONTAINER}:/opt/iocURLDriver/  ${HOST_IOC_ROOT}
mkdir -p ${OP_DIR}
docker cp ${CONTAINER}:/opt/screens/   ${OP_DIR}

# edit files in docker container IOC for use with GUI software
echo "changing 13URL1: to ${PREFIX} in ${CONTAINER}"
sed -i s+13URL1+`echo ${PRE}`+g ${IOC_TOP}/start_caQtDM_adurl
sed -i s+_IOC_SCREEN_DIR_+`echo ${IOC_TOP}`+g ${IOC_TOP}/start_caQtDM_adurl
sed -i s+_AD_SCREENS_DIR_+`echo ${OP_DIR}/screens/ui`+g ${IOC_TOP}/start_caQtDM_adurl
sed -i s+"# CAQTDM_DISPLAY_PATH"+CAQTDM_DISPLAY_PATH+g ${IOC_TOP}/start_caQtDM_adurl
